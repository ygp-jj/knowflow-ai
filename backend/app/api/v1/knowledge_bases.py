...
def create(
    ...
    参数:
        payload: 创建知识库请求体，包含 name、description。
        db: FastAPI 注入的数据库会话。
        current_user: 当前登录用户。
    返回:
    ...
    knowledge_base = create_knowledge_base(db, payload, owner_id=current_user.id)
...
def list_items(
    ...
    items, total = list_knowledge_bases(db, current_user.id, page, page_size)
    data = {
...
def detail(
    ...
    knowledge_base = get_knowledge_base(db, id, current_user.id)
...
def update(
    ...
    knowledge_base = update_knowledge_base(db, payload, owner_id=current_user.id)
    if knowledge_base is None:
        return error_response(404, "知识库不存在")
...
def delete(
    ...
    result = delete_knowledge_base(db, id, current_user.id)
    if result == "not_found":
        return error_response(404, "知识库不存在")
