...
- 演示账号仅保留 `hr_admin`（id=101）
- 演示知识库 / 文档 / 会话均归属该账号
- 适合前端页面直接演示
- 适合知识库、文档、聊天、引用来源、反馈、评测页面联动展示
- **演示登录**：`hr_admin` / `demo123456`（bcrypt）；也可在登录页注册新用户
...
整套演示数据围绕 2 个制度主题展开（均归属 `hr_admin`）：
...
- **登录鉴权**：仅保留演示用户 `hr_admin`，密码 `demo123456`（bcrypt）。重跑本 seed 会删除 `finance_admin` / `ops_admin` / `employee_demo` 及其关联知识库数据。若仅更新密码可执行：
  `UPDATE users SET hashed_password = '$2b$12$jLRQnn2kAn3atfpId52Xsuxk58qSlsblfVt4mDxlwZEFHTyNbYP..' WHERE id = 101;`
